  ...
  before :each do
    ...
    describe 'GET #index' do
      it "populates an array of all vouchers" do
        voucher1 = create(:voucher, code_name: "FIRSTCODE")
        voucher2 = create(:voucher, code_name: "SECONDCODE")
        get :index
    ...
    context "with valid attributes" do
      ...
      it "changes @voucher's attributes" do 
        patch :update, params: { id: @voucher, voucher: attributes_for(:voucher, code_name: 'HEMAT7K')}
        @voucher.reload
        expect(@voucher.code_name).to eq('HEMAT7K')
      end
    ...
    context "with invalid attributes" do
      it "does not update the voucher in the database" do
        patch :update, params: { id: @voucher, voucher: attributes_for(:voucher, code_name: 'HEMAT7K', valid_from: nil) }
        @voucher.reload
        expect(@voucher.code_name).not_to eq('HEMAT7K')
      end

      it "re-renders the :edit template" do 
        post :update, params: { id: @voucher, voucher: attributes_for(:invalid_voucher) }
        expect(response).to render_template :edit
      end
    end
  end
